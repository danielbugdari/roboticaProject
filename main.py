...
class PrintingRobot():
    ...
    def camera_controller(self):
        ...
        def execute(color):
            ...
            """
            if color == 1: # red
                self.ev3.screen.draw_text(40, 50, "print red - print Rectangle")
                self.print(configuration.print_shapes['Rectangle'])
            elif color == 2: # blue
                self.ev3.screen.draw_text(40, 50, "print Blue - print Triangle")
                self.print(configuration.print_shapes['Triangle'])
            elif color == 4: # green
                self.ev3.screen.draw_text(40, 50, "print green - print House")
        ...
        while running:
            if self.button.pressed():
                self.ev3.screen.draw_text(40, 50, "Button was pressed")
                start_time = time.time()
                while self.button.pressed():
                    color = (self.camera.read('TRACK'))[1]  # take only the color argument
                self.ev3.screen.draw_text(40, 50, "color is {}".format(color))
                end_time = time.time()

                if (end_time - start_time) > 5: # Turn off
                    self.ev3.screen.draw_text(40, 50, "turn off camera mode")
                    running = False
                else:  # execute printing
                    self.ev3.screen.draw_text(40, 50, "execute printing by color")
                    execute(color)
    ...
    def clicks_controller(self):
        ...
        self.ev3.screen.draw_text(40, 50, "Inside clicks moode")
        ...
        while running:
            ...
            if self.button.pressed():
                self.ev3.screen.draw_text(40, 50, "btn is prsd")
                start_time = time.time()  # time in seconds
                clicks += 1  # add click counter
                while self.button.pressed(): # for case of long pressing
                    self.ev3.screen.draw_text(40, 50, "prsd down")
                    pass
                end_time = time.time()
    ...
    def set_mode(self):
        ...
        def execute(number_of_clicks):
            ...
            This function executes printing according to amount of pressing on the touchSensor
            """
            self.ev3.screen.draw_text(40, 50, "inside set_mode -> execute {} clicks".format(number_of_clicks))
            if number_of_clicks == 0:
                return False
        ...
        while running:
            ...
            if self.button.pressed():
                ...
                clicks += 1  # add click counter
                while self.button.pressed():  # for case of long pressing
                    self.ev3.screen.draw_text(40, 50, "button is pressed")
                    pass
                end_time = time.time()
                
                if (end_time - start_time) > end_timeout_seconds:
                    self.ev3.screen.draw_text(40, 50, "turning off")
                    running = False
                    clicks = 0
